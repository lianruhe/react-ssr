...
const appEntry = ['core-js/shim', paths.web('index.js')]
if (__DEV__) {
  appEntry.unshift(
      ...
      plugins: function () {
        return [
          ...
          require('postcss-cssnext')({
            features: {
              customProperties: {
                variables: require(paths.web(`themes/${config.theme}/variables.json`))
              }
            }
          })
        ]
...
const webpackConfig = {
  target: 'web',
  resolve: {
    modules: [paths.web(), 'node_modules'],
  ...
  entry: {
    app: appEntry,
    vendor: config.compiler_web_vendor
  },
  output: {
    path: paths.dist(),
...
if (!__TEST__) {
  webpackConfig.plugins.push(
    new FaviconsWebpackPlugin({
      logo: paths.web('static/favicon.png'),
